...
| UI-LAYOUT-04 | 分栏 | inferred | N/A；参考图装备与手法相邻 | 装备 2fr、侧栏 1fr / 同采用值（已实现） | core | 表格与侧栏顶对齐 |
| UI-LAYOUT-05 | 内边距 | inferred | N/A；新表格密度 | 页头 24px，单元格 8px 12px，说明 16px / 同采用值（已实现） | warning | 对齐与可读性 |
| UI-LAYOUT-06 | 间距 | inferred | N/A；新并排元素 | 图文 gap 10px，控制区 gap 8px / 同采用值（已实现） | warning | 图文不重叠 |
...
| UI-TYPOGRAPHY-03 | 行高 | source | globals.css :root --text-small-line 1.65 | 1.65 / 同采用值（已实现） | warning | 实现检查 |
...
| UI-ASSET-02 | 图片尺寸 | inferred | N/A；紧凑表格图文 | 36×44 装备框、32×32 技能、24×24 宝石 / 同采用值（已实现） | warning | contain 不变形 |
...
最终生产构建成功，48 项测试全部通过（含新增表格路由 SSR）。
...
- 导出测试替换了 DOM 到 PNG 的浏览器边界，验证的是实际队列与 ZIP 字节，并未宣称完成浏览器真实图片下载或截图视觉验收。按 Sites 环境规则，本次未运行用户未要求的浏览器交互测试。inferred 视觉项经 CSS/DOM 静态核对，视觉核对保留 warning。
- 本地预览 /builds/tragoul-nova?view=table 返回 HTTP 200，已向 Codex 打开预览。
- 全仓 TypeScript 检查仍有既有错误：crusader-builds pushNote 字段、necromancer-builds checkpoint reason、旧页面 summary / guide 引用、Cloudflare 类型声明缺失；新增组件无 TypeScript 错误。
- 新增导出模块与表格模块 ESLint 无错误；保留原生 img 以适配本地游戏素材和 DOM 导出（Next 图片优化提示属 warning）。

补充映射：UI-LAYOUT-05 页头 gap 24px、行内 6px、状态区 12px；UI-TYPOGRAPHY-01 主标题 32px/1.4、分区 18px/1.4、辅助说明 12px/1.5，均为新表格内部 inferred 分级，正文与常用标签保持 16px / 14px。UI-ASSET-02 符文沿用 globals.css .rune-icon 的 42×42 及 sprite 坐标。UI-INTERACTION-03 离屏宿主 absolute left:-100000px top:0 width:1440px，aria-hidden、不参与页面布局、finally 清理。所有新增 CSS 限定在 bd-sheet / bd-table 前缀下。
