...
塔格奥死亡新星不是只有“冲层/速刷”两套配置。当前资料明确区分了 GR 冲层、GR 速刷和 T16 小秘境，项目原先把后两者合并是实质性错误。复审又删除了按高低巅峰复制出来的三对场景：现在只保留一个奥吉德 GR 冲层、一个 GR 速刷和一个 T16 研究场景。守护者改为 GR 速刷场景内的条件化过渡，不再外推成没有精确来源的冲层配置，也不再把“约 2000”当作产品硬断点。
...
裁决：只保留一个 T16 研究配置，适用性必须是 `unverified`；完成游戏内装配与循环验证前不可显示为推荐。
...
| --- | --- | --- | --- | --- |
| `gr-push` | GR 冲层 | `supported` | 奥吉德肩腕、戴恩提、导能披肩萃取；困者/贼神/受罚者 | 装备、宝石、魔方双源一致；技能符文与 Switch 操作仍未通过 |
| `gr-speed` | GR 速刷 | `viable` | 成型配置用奥吉德与戴恩提；斯图亚特萃取、强者替代受罚者；站不住时才考虑守护者过渡 | 资料明确“合理可用”，没有证据称同职业最快；守护者转奥吉德的完整条件只有 Icy 单源 |
| `t16-rift` | T16 小秘境 | `unverified` | 六件塔格奥、沃兹克、金织带、布里格斯；单人随从带贪婪之戒 | 金币链有 Icy 支持，但第二来源不一致，且技能栏与 S39 第四槽存在页内冲突 |
...
3. 单人 T16 研究配置将贪婪之戒交给随从，玩家第四槽改为斯奎特；避免玩家同时穿戴/萃取错误的金币戒逻辑，但在冲突裁决前不把它标成推荐。
4. 修复详情解析优先级：场景 `configurationBase + patch` 现在优先于旧 `resolveGear/resolvePowers/resolveRotation`，否则按钮虽然切换，展示仍会落回旧四格配置。
5. 删除没有精确来源的“守护者冲层”，并将守护者保留为 GR 速刷内的条件化成长策略。
6. 删除高/低巅峰复制场景，用换装后生存、目标层稳定性和精英击杀瓶颈作为切换条件。
7. 来源迁入结构化对象，并以结论为单位记录支持、冲突和发布阻塞。
...
- [ ] 技能冲突裁决
- [ ] T16 第四槽冲突裁决
- [x] 成长建议改为可观察条件，不再使用固定 2000 巅峰切换
- [ ] Nintendo Switch 三次功能样本
- [ ] 三场景收敛后的桌面、移动端与英文视觉复核
