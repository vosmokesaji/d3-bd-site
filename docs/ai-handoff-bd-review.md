...
这是一份可独立使用的续作说明。2026-09-12 内容准确性复审推翻了“51 套均已完成”的旧结论：旧校验只能证明结构可解析，不能证明配置正确。接手者应以 [BD 准确性整改执行任务](./bd-accuracy-execution-tasks.md) 和 `docs/bd-review-audit.json` 为当前唯一状态基线。
...
| `source-checked` | 12 | 塔格奥、梦遗轰击、祖尼玛毒镖、梦遗新星与拉斯玛已有结论级证据但仍有冲突；7 套批量魔法师仍待第二来源与用途拆分 |
| `unverified` | 38 | 包含 21 套通用工厂占位数据及缺少显式证据状态的旧手写数据 |

当前工作游标：D04 `h90-frenzy` 九十蛮狂乱。用户指定的塔格奥新星、梦遗新星、拉斯玛大军三项优先队列已完成来源级整改。结构有效 51/51、现有语义规则有效 51/51、发布级验证 0/51；来源冲突与 Switch 实测作为显式阻塞保留。

旧流程曾为死灵 7 套、魔法师 7 套及若干野蛮人、圣教军和巫医条目录入完整场景结构，但这些记录现在只能视为“已有待审数据”，不能写成内容完成。具体历史覆盖范围保留在后文日志，用于定位代码。

建议后续动作：

1. 执行 D04 `h90-frenzy` 的精确来源核验与用途拆分。已有来源级 BD 等待可读的独立来源与 Switch 实测时保持当前证据状态。
...
- 通用 BD 详情页已经从每套 `scenarios` 动态生成用途按钮，并能根据场景配置切换装备、普通/传奇宝石、技能、被动、卡奈魔方和实战循环。
- `components/items/DiabloItemFrame.tsx` 是装备框统一底层组件。
- 随从装备盘、空槽、技能区和多视口布局已经完成。
- 全站字号/行高 token 与关键视觉回归已经完成。
- 当前自动化把结构、语义、证据和可发布状态分开统计；不要再用测试通过代替内容准确。

旧完成记录只说明曾经录入过结构和分支，不代表内容已经交叉验证。下文历史提交与逐套日志可以帮助定位代码，但其中“已校对”“已完成”均应按新证据门重新审核。
...
| D02 | `lod-bombardment` | `source-checked / platform-risk` | `50d6961` | 三场景、6 个结构化来源、14 条结论证据、双手/副手语义规则、条件化成长、桌面/移动 QA | 技能/装备/宝石冲突、Maxroll 正文、Switch 实测 |
...
| 34 | `lod-nova` | `source-checked / platform-risk` | `87c5f73` | 两个GR场景、7个结构化来源、15条结论证据；修正白宝石、鲜血灌注、护肩词缀与巅峰顺序 | Maxroll正文、Icy旧文裁决、完整配装冲突、Switch实测与页面QA |
| 35 | `inarius-nova` | 已校对 | `feat(bd): 完成死灵法师新星系多场景校对` | 伊纳瑞斯骨甲旋风、轮回镰刀/寅剑 | 仅回归维护 |
| 36 | `rathma-aotd` | `source-checked / platform-risk` | 本轮待提交 | 三场景、14 个结构化来源、18 条结论证据；修复蚀牙断链、Icy 轮回镰刀错误与 T16 穿戴拼装 | 精确符文/宝石冲突、Maxroll 正文、Switch 实测与浏览器 QA |
