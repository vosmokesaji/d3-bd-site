...
| 状态 | 数量 | BD |
| --- | ---: | --- |
| 已完整校对 | 2 | `tragoul-nova`、`wastes-rend` |
| 部分实装 | 2 | `pony-fist-farm`、`god-monk` |
| 尚未逐套校对 | 47 | 见路线图第 9 节清单 |
| 仍需完成 | 49 | 2 套部分实装 + 47 套草稿 |

当前工作游标：路线图清单第 3 项 `pony-fist-farm`。

建议后续顺序：

1. `pony-fist-farm`：补齐并完成当前部分实装。
2. `god-hungering`：按清单顺序建立猎魔人第二个完整样板。
3. `god-monk`：把已有两套配装与真实用途、巅峰场景连接起来。
4. 继续按照 `docs/bd-content-and-ui-roadmap.md` 第 9 节顺序逐套推进。
...
- 最近一次完整验证为 `npm test` 22 项全部通过。

已知部分实装的真实含义：

- `pony-fist-farm` 目前只有 `powerSets` 按用途改变护甲萃取；装备、技能、宝石、普通宝石、巅峰、固定/替换策略和完整场景契约尚未完成。
...
目标 ID：`pony-fist-farm`

主要文件：

- `app/data/crusader-builds.ts`
- `app/data/site-catalog.ts`
- `app/data/build-guides.ts`，仅在需要扩展真实内容类型时修改
- `tests/rendered-html.test.mjs`
- 本文件与 `docs/bd-content-and-ui-roadmap.md`

当前已有内容：

- 勇气壁垒、诺瓦德、黑暗之光、正义腰带、斯奎特、乔丹、瑞秋等基础装备。
- 天堂之拳、战马冲锋、希望律法、钢铁之肤、挑衅、阿卡拉特勇士等技能。
- `push` 与 `speed` 两组 `powerSets`，目前主要区别是守夜人腰带与金织带萃取。
- 页面文案声称支持 T16 小秘境、悬赏、蓝门和 110 层以下大秘境。

必须补齐：

- 蓝门、低层大秘境、T16 小秘境和悬赏是否能合并场景的资料判断。
- 每个真实场景的完整装备、技能、被动、威能、宝石、普通宝石、词缀和循环。
- 低/高巅峰的生存、伤害、冷却和移动差异。
- 金币链是否成立：金织带、囤宝者、贪婪之戒在哪些内容中有效，哪些内容不能依赖金币。
- 斯奎特、乔丹、瑞秋、沃兹克、守夜人等槽位的固定程度和替换条件。
- 第 39 赛季第四魔方槽中寅剑的适用边界。
- 蓝门高波次与 T16/悬赏是否需要不同技能符文或保命方案。
- 随从在大秘境、T16、悬赏和敌意幻象中的装备区别。

重要建模提醒：项目文案里的“蓝门”指敌意幻象（Visions of Enmity），不是大秘境。当前 `BuildContent` 没有这一类型。若研究确认需要单独建模，应把 `"vision-of-enmity"` 加入 `BuildContent`，并同步 UI 标签、测试和路线图示例；禁止为了省事把蓝门错误标成 `greater-rift-speed`。

另一个关键判断是四格场景是否够用。若蓝门、T16 和悬赏的完整配置确实不同，就扩展为真实场景选择，不要用一个 `speed-low` 同时冒充三套配置。只有资料和逐项比较证明配置一致时，才允许合并，并在 `unchangedReason` 或场景原因中说明共同条件。

跑马天拳完成后，预期结果：

- 目标 guide 通过 `validateReviewedBuildGuide`。
- 路由切换展示真实配置差异，不再只有护甲萃取和说明文字变化。
- `reviewStatus` 为 `fully-reviewed`。
- 49 套剩余更新为 48 套；部分实装从 2 套更新为 1 套；完整校对从 2 套更新为 3 套。
- 下一工作游标更新为路线图第 4 项 `god-hungering`。
...
| 3 | `pony-fist-farm` | 部分 | 尚无完成提交 | 旧 `powerSets` 区分两种护甲威能 | 按第 6 节完成真实内容建模 |
| 4 | `god-hungering` | 待校对 | 无 | 基础 guide | 跑马天拳完成后开始 |
