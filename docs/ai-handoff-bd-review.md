...
| `source-checked` | 8 | 塔格奥样板与 7 套批量魔法师，仍待第二来源或用途拆分 |
| `unverified` | 42 | 包含 23 套通用工厂占位数据及缺少显式证据状态的旧手写数据 |

当前工作游标：G0 止损已经完成，正在执行 G1 数据与证据模型。结构有效 51/51、现有语义规则有效 51/51、发布级验证 0/51；下一阶段按五套试点而不是旧编号顺序推进。

旧流程曾为死灵 7 套、魔法师 7 套及若干野蛮人、圣教军和巫医条目录入完整场景结构，但这些记录现在只能视为“已有待审数据”，不能写成内容完成。具体历史覆盖范围保留在后文日志，用于定位代码。

建议后续动作：

1. 继续完成执行任务中的 G1，再处理塔格奥、梦遗轰击、祖尼玛毒镖、九十蛮狂乱和火鸟爆炸冲击五套试点；塔格奥证据档案已经建立。
