...
| `source-checked` | 9 | 塔格奥与梦遗轰击已有结论级证据但仍有冲突；7 套批量魔法师仍待第二来源与用途拆分 |
| `unverified` | 41 | 包含 22 套通用工厂占位数据及缺少显式证据状态的旧手写数据 |

当前工作游标：D03 祖尼玛毒镖。结构有效 51/51、现有语义规则有效 51/51、发布级验证 0/51；D01 塔格奥与 D02 梦遗轰击已完成来源级代码迁移，分别保留来源冲突和 Switch 实测阻塞，不再强行补齐固定四象限。

旧流程曾为死灵 7 套、魔法师 7 套及若干野蛮人、圣教军和巫医条目录入完整场景结构，但这些记录现在只能视为“已有待审数据”，不能写成内容完成。具体历史覆盖范围保留在后文日志，用于定位代码。

建议后续动作：

1. 继续 D03 祖尼玛毒镖，重点验证宠物攻速、智力职业普通宝石与条件化成长；D01/D02 等待可读的独立来源与 Switch 实测时保持当前证据状态。
...
| 巫医与下一套 D03 | `app/data/witch-doctor-builds.ts` |
