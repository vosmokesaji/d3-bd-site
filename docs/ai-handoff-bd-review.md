...
| `source-checked` | 8 | 塔格奥已完成部分结论双源但仍有冲突；7 套批量魔法师仍待第二来源与用途拆分 |
| `unverified` | 42 | 包含 23 套通用工厂占位数据及缺少显式证据状态的旧手写数据 |

当前工作游标：D01 塔格奥样板收口。结构有效 51/51、现有语义规则有效 51/51、发布级验证 0/51；塔格奥已有 6 个结构化来源、10 条结论证据和 6 个真实用途/成长场景，但技能、T16 第四槽与 Switch 实测仍阻止升级。

旧流程曾为死灵 7 套、魔法师 7 套及若干野蛮人、圣教军和巫医条目录入完整场景结构，但这些记录现在只能视为“已有待审数据”，不能写成内容完成。具体历史覆盖范围保留在后文日志，用于定位代码。

建议后续动作：

1. 先裁决塔格奥技能/T16 冲突并完成 Switch 实测记录；同时继续 G1 的全量迁移，再处理梦遗轰击、祖尼玛毒镖、九十蛮狂乱和火鸟爆炸冲击试点。
