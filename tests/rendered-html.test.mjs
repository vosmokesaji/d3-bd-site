...
test("renders the reviewed Akkhan Phalanx scenarios with pet and self-cast branches", async () => {
  ...
  for (const runtimeChoice of ["AKKHAN_PHALANX_CONFIGURATION_BASE", "AKKHAN_PHALANX_SOURCES", "kassar-helm", "tasker", "unrelenting", "frydehr", "goldwrap-power", "messerschmidt", "boon-of-the-hoarder"]) {
    assert.match(crusaderData, new RegExp(runtimeChoice));
  }
  assert.match(crusaderData, /validateReviewedBuildGuide\(AKKHAN_PHALANX_REVIEWED_GUIDE\)/);
});
